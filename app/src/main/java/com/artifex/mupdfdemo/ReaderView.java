...
public class ReaderView
	...
	public ReaderView(Context context, AttributeSet attrs) {
		...
		// "Edit mode" means when the View is being displayed in the Android GUI editor. (this
		// class
	...
	public void onLongPress(MotionEvent e) {
		//注释 onDown->onShowPress->onLongPress
		//注释 长按触摸屏，超过一定时长，就会触发这个事件
	}
	...
	public void onShowPress(MotionEvent e) {
		//注释 如果是按下的时间超过瞬间，而且在按下的时候没有松开或者是拖动的，那么onShowPress就会执行
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		//注释  点击时 出现可以编辑已绘制的区域 及用户交互行为
		if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_DOWN) {
			mUserInteracting = true;
			LogUtils.d(TAG,"(& MotionEvent.ACTION_MASK) == MotionEvent.ACTION_DOWN)");
		}
		if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP) {
			mUserInteracting = false;
			LogUtils.d(TAG,"(MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP)");

			View v = mChildViews.get(mCurrent);
			if (v != null) {
				if (mScroller.isFinished()) {
			LogUtils.d(TAG,"slideViewOntoScreen");
